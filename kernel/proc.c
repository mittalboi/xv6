...
void scheduler(void)
{
  ...
  for (;;)
  {
    ...
    for (p = proc; p < &proc[NPROC]; p++)
    {
      ...
      if (p->state == RUNNABLE)
      {
        ...
        if (p->create_time < TempProc->create_time)
        {
          TempProc = p;
        }
      }
  ...
  for (;;)
  {
    ...
    int winner = randomrange(1, total_tickets);
    // printf("winner = %d\n", winner);
    struct proc *tobescheduled = 0;
    int flag = 0;
    for (p = proc; p < &proc[NPROC]; p++)
    {
      // acquire(&p->lock);
  ...
  for (;;)
  {
    ...
    for (p = proc; p < &proc[NPROC]; p++)
    {
      if (p->rtime == 0 && p->time_stopped == 0)
        ...
        p->niceness = (p->time_stopped) / (p->time_stopped + p->rtime);
        p->niceness = 10 * p->niceness;
      }
      p->dynamic_priority = p->static_priority - p->niceness;
      p->dynamic_priority = p->dynamic_priority + 5;
      ...
      if (p->state == RUNNABLE)
      {
        ...
        else if (p->dynamic_priority < TempProc->dynamic_priority)
        {
          TempProc = p;
        }
        else if (TempProc->dynamic_priority == p->dynamic_priority)
        {
          if ( p->times_chosen > TempProc->times_chosen)
          {
            TempProc = p;
          }
